...
def rf_pipeline(co, den, sp, splits, best_param):
    ...
    for train_index, test_index in kf.split(X):
        ...
        X_train_crs, X_test_crs, y_train, y_test = cudf.from_pandas(X_train), cudf.from_pandas(X_test),cudf.from_pandas(y_train), cudf.from_pandas(y_test)
        print('Data prepared')
        ######### Random Forest #########
        ...
        results = pd.DataFrame({'mse': mse,'rmse': rmse ,'r2': r2,'mae': mae,'time': runtime}, index=[splits])
        results_list.append(results)
    # save the results as a csv file
    # Combine the results for all folds
    final_results = pd.concat(results_list) 
    result = final_results.mean()
    if co and sp:
        result.to_csv(folder2 + '/RF_spco/'+ field +'all'+ '.csv')
    elif not co and sp:
        result.to_csv(folder2 + '/RF_sp/'+ field + 'all'+ '.csv')
    else:
        result.to_csv(folder2 + '/RF/'+ field + 'all'+ '.csv')
    print('Results saved')
...
for i in range(len(fields)):
    ...
    if not study_exists:
        spatial_hyperparameter(path=path, path_b=path_b, dgm_files=dgm_files, aspect_files=None, field=field, co=True, sp=True, split=splits, vista=vista)
